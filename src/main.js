...
function createWindow() {
    ...
    // Web MIDI) are allowed for any tab; the sensitive ones (microphone/camera,
    // location) are restricted to x.com/twitter.com so an arbitrary site opened
    // in the built-in browser can't silently capture them.
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
        // Low-risk permissions are allowed anywhere in the app. 'midi'/'midiSysex'
        // are here because webview-preload.js injects the mixer overlay on EVERY
        // page, so a hardware MIDI controller has to keep working outside X tabs
        // too — scoping these to x.com would break the app-wide Sounds rig.
        const globallyAllowed = [
            'notifications', 'pointerLock', 'clipboard-read', 'clipboard-sanitized-write',
            'midi', 'midiSysex',
        ];
            ...
            submenu: [
                {
                    ...
                    click: () => {
                        dialog.showMessageBox(mainWindow, {
                            type: 'info',
                            title: 'About XCaster',
                            message: 'XCaster v1.4.7',
                            detail:
                                'Standalone Windows shell for x.com with WebRTC AGC, noise suppression, and echo cancellation disabled.\n\n' +
                                'v1.4.7 — Removes a false "incoming Space audio has stopped" warning that could appear during a perfectly healthy Space, along with the automatic reconnect it triggered. Also stops several background checks from repeating work every few seconds, including one that could interrupt the broadcast of whoever is hosting.\n\n' +
